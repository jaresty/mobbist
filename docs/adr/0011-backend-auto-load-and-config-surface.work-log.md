...
## loop-10 | 2026-01-22T21:34:00Z | helper:v20251223.1
- helper_version: helper:v20251223.1
- focus: ADR-0011 (Decision section 2) – render backend controls as a compact popover to reduce visual weight.
- work_log_updated: `docs/adr/0011-backend-auto-load-and-config-surface.work-log.md` (loop-10)
- active_constraint: Backend controls still expand full-width, keeping configuration visually dominant; `npm test` fails on the new popover guardrail.
- expected_value: Impact=Medium (backend feels optional), Probability=High (CSS/markup update), Time Sensitivity=Low (polish); uncertainty: low after green.
- validation_targets:
  - `npm test`
- mitigation_ladder: 1) add specifying test for popover render; 2) switch drawer to popover layout; 3) add mobile fallback for small screens.
- evidence:
  - red: `docs/adr/evidence/0011/loop-10.md#loop-10-red`
  - green: `docs/adr/evidence/0011/loop-10.md#loop-10-green`
  - removal: `docs/adr/evidence/0011/loop-10.md#loop-10-removal`
- rollback_plan: `git restore --source=HEAD -- index.html tests/backend-status.test.js docs/adr/evidence/0011/loop-10.md` then re-run `npm test` to observe the popover regression.
- delta_summary: helper:diff-snapshot=`index.html | 25 insertions(+), 2 deletions(-); tests/backend-status.test.js | 10 insertions(+)`; drawer now renders as a popover with mobile fallback; wip preserved at `docs/adr/evidence/0011/loop-10-wip.patch` for removal check.
...
## loop-11 | 2026-01-22T21:45:20Z | helper:v20251223.1
- helper_version: helper:v20251223.1
- focus: ADR-0011 (Decision sections 2-4) – make backend pill the configure control, place it near the header, and embed heartbeat state as an emoji.
- work_log_updated: `docs/adr/0011-backend-auto-load-and-config-surface.work-log.md` (loop-11)
- active_constraint: Backend status still reads as a separate control block; pill placement + emoji heartbeat were not enforced by tests, so regressions could restore the prominent bar. `npm test` fails on new guardrails until the layout and copy align.
- expected_value: Impact=Medium (backend feels optional), Probability=High (layout + copy updates), Time Sensitivity=Low (polish); uncertainty: low after green.
- validation_targets:
  - `npm test`
- mitigation_ladder: 1) add specifying tests for header pill + emoji status; 2) relocate backend status into header; 3) use the pill as the configure action with popover anchored to it.
- evidence:
  - red: `docs/adr/evidence/0011/loop-11.md#loop-11-red`
  - green: `docs/adr/evidence/0011/loop-11.md#loop-11-green`
  - removal: `docs/adr/evidence/0011/loop-11.md#loop-11-removal`
- rollback_plan: `git restore --source=HEAD -- index.html tests/backend-status.test.js docs/adr/evidence/0011/loop-11.md` then re-run `npm test` to observe the header pill regression.
- delta_summary: helper:diff-snapshot=`index.html | 105 insertions(+), 61 deletions(-); tests/backend-status.test.js | 40 insertions(+), 14 deletions(-)`; backend pill now serves as configure action near header with emoji heartbeat; wip preserved at `docs/adr/evidence/0011/loop-11-wip.patch` for removal check.
- loops_remaining_forecast: 0–1 loops (manual smoke evidence logging or heartbeat cadence coverage); confidence: Medium.
- residual_constraints:
  - Heartbeat cadence (30s) is not specified by tests; only state labels are covered. Mitigation: add a timer-based test or contract assertion for the interval. Severity: Medium. Trigger: heartbeat scheduling changes. Owner: `docs/adr/0011-backend-auto-load-and-config-surface.md`.
  - Manual smoke with live backend is not logged in evidence. Mitigation: capture a manual smoke run transcript in ADR evidence. Severity: Medium. Trigger: backend rollout or API changes. Owner: `docs/adr/0010-backend-contract.md`.
...
## loop-14 | 2026-01-22T22:04:20Z | helper:v20251223.1
- helper_version: helper:v20251223.1
- focus: ADR-0011 (Decision section 4) – specify heartbeat cadence via fake timers and test hooks.
- work_log_updated: `docs/adr/0011-backend-auto-load-and-config-surface.work-log.md` (loop-14)
- active_constraint: Heartbeat cadence lacked a specifying test; interval changes could regress silently. `npm test` failed on the new cadence guardrail until heartbeat helpers were exposed.
- expected_value: Impact=Medium (keeps heartbeat cadence stable), Probability=High (test + hooks), Time Sensitivity=Low (quality); uncertainty: low after green.
- validation_targets:
  - `npm test`
- mitigation_ladder: 1) add specifying cadence test; 2) expose heartbeat helpers for tests; 3) assert 30s interval with fake timers.
- evidence:
  - red: `docs/adr/evidence/0011/loop-14.md#loop-14-red`
  - green: `docs/adr/evidence/0011/loop-14.md#loop-14-green`
  - removal: `docs/adr/evidence/0011/loop-14.md#loop-14-removal`
- rollback_plan: `git restore --source=HEAD -- index.html tests/backend-status.test.js docs/adr/evidence/0011/loop-14.md` then re-run `npm test` to observe the cadence regression.
- delta_summary: helper:diff-snapshot=`index.html | 2 insertions(+); tests/backend-status.test.js | 19 insertions(+)`; heartbeat cadence now specified by fake-timer test; wip preserved at `docs/adr/evidence/0011/loop-14-wip.patch` for removal check.
